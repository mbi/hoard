...
def render_field(parser, token):
    ...
    """
    error_msg = (
        f"{token.split_contents()[0]!r} tag requires a form field followed by a list of attributes and "
        'values in the form attr="value"'
    )
    try:
    ...
    def render(self, context):
        ...
        if getattr(bounded_field, "errors", None) and "WIDGET_ERROR_CLASS" in context:
            bounded_field = append_attr(
                bounded_field, "class:{}".format(context["WIDGET_ERROR_CLASS"])
            )
        if field and field.required and "WIDGET_REQUIRED_CLASS" in context:
            bounded_field = append_attr(
                bounded_field, "class:{}".format(context["WIDGET_REQUIRED_CLASS"])
            )
